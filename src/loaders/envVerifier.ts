    ...
    try {
        ...
        for await (const line of readIntSampleENV) {
            ...
            if (
                ...
                if (
                    !envObject[`${envVar}`] ||
                    envObject[`${envVar}`].trim() == ''
                ) {
                    // env key doesn't exist, ask for input
                    LoggerInstance.input(
                        `  Enter ENV Variable Value --> ${envVar}`
                    )
                    ...
                    while (value.trim().length == 0) {
                        ...
                        value = await doSyncPrompt(
                            rl,
                            `${envSampleObject[envVar]} >`
                        )

                        if (value.trim().length == 0) {
                            LoggerInstance.error(
                                "  Incorrect Entry, Field can't be empty"
                            )
                        }
                    }
